function uncertainty = computePointUncertainty(point, cameras, numCams, Resolution, adjacentSurfaces, du,dv, penaltyUncertainty)
    uv = zeros(numCams,2);
    ...
    for i = 1:numCams
        ...
        camCenter = camCenter(:); % Ensure column vector
        if (u >= 1 && u <= Resolution(1) && v >= 1 && v <= Resolution(2))
            worldPoints = quantToWorld(cameras{i}, u, v, du, dv, camCenter);
            planes{i} = buildPyramidSurf(camCenter, worldPoints, adjacentSurfaces);
            visIdx(i) = true;
        end
    end
